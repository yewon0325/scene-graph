...
class Transformer(nn.Module):
    ...
    def _prepare_inputs(self, src, mask, entity_embed, triplet_embed, pos_embed):
        ...
        mask = mask.flatten(1)
        return src, mask, entity_embed, entity, triplet_embed, triplet, pos_embed, bs, c, h, w

    def prepare_triplet_queries(self, triplet_embed, batch_size):
        c = self.d_model
        triplet_pos, triplet = torch.split(triplet_embed, [c, 2 * c], dim=1)
        triplet_pos = triplet_pos.unsqueeze(1).repeat(1, batch_size, 1)
        triplet = triplet.unsqueeze(1).repeat(1, batch_size, 1)
        return triplet, triplet_pos
    ...
    def forward(self, src, mask, entity_embed, triplet_embed, pos_embed, so_embed):
        src, mask, entity_embed, entity, triplet_embed, triplet, pos_embed, bs, c, h, w = \
            self._prepare_inputs(src, mask, entity_embed, triplet_embed, pos_embed)

        memory = self.encode(src, mask, pos_embed)
        if memory.is_cuda:
            torch.cuda.synchronize(memory.device)
        entity_start = time.perf_counter()
        entity_states = self.decoder.forward_entity(
            entity, memory, memory_key_padding_mask=mask, pos=pos_embed, entity_pos=entity_embed
        )
        if memory.is_cuda:
            torch.cuda.synchronize(memory.device)
        triplet_start = time.perf_counter()
        hs_t, sub_maps, obj_maps = self.decoder.forward_triplet(
            triplet, entity_states, memory, memory_key_padding_mask=mask,
            pos=pos_embed, triplet_pos=triplet_embed, so_pos=so_embed
        )
        if memory.is_cuda:
            torch.cuda.synchronize(memory.device)
        timing_stats = {
            'entity_decoder_time_ms': (triplet_start - entity_start) * 1000.0,
            'relation_decoder_time_ms': (time.perf_counter() - triplet_start) * 1000.0,
        }
        ...
        return entity_states.transpose(1, 2), hs_t.transpose(1, 2), so_masks, memory.permute(1, 2, 0).view(bs, c, h, w), timing_stats
...
class TransformerDecoder(nn.Module):
    ...
    def forward_entity(self, entity, memory,
        ...
        for layer in self.layers:
            output_entity = layer.forward_entity(output_entity, entity_pos, memory, tgt_mask=tgt_mask,
        ...
        return output_entity.unsqueeze(0)

    def forward_triplet(self, triplet, entity_states, memory,
                        tgt_mask: Optional[Tensor] = None,
                        memory_mask: Optional[Tensor] = None,
                        tgt_key_padding_mask: Optional[Tensor] = None,
                        memory_key_padding_mask: Optional[Tensor] = None,
                        pos: Optional[Tensor] = None, triplet_pos: Optional[Tensor] = None,
                        so_pos: Optional[Tensor] = None):
        ...
        for layer_id, layer in enumerate(self.layers):
            tgt_entity = entity_states[layer_id]
            output_triplet, sub_maps, obj_maps = layer.forward_triplet(
                output_triplet, tgt_entity, triplet_pos, so_pos, memory,
        ...
        if self.return_intermediate:
            return torch.stack(intermediate_triplet), torch.stack(intermediate_submaps), torch.stack(intermediate_objmaps)
        return output_triplet.unsqueeze(0), sub_maps.unsqueeze(0), obj_maps.unsqueeze(0)
    ...
    def forward(self, entity, triplet, memory,
        ...
        entity_states = self.forward_entity(
            entity, memory, tgt_mask=tgt_mask, memory_mask=memory_mask,
            tgt_key_padding_mask=tgt_key_padding_mask,
            memory_key_padding_mask=memory_key_padding_mask, pos=pos, entity_pos=entity_pos
        )
        triplet_states, sub_maps, obj_maps = self.forward_triplet(
            triplet, entity_states, memory, tgt_mask=tgt_mask, memory_mask=memory_mask,
            tgt_key_padding_mask=tgt_key_padding_mask,
            memory_key_padding_mask=memory_key_padding_mask, pos=pos,
            triplet_pos=triplet_pos, so_pos=so_pos
        )
        return entity_states, triplet_states, sub_maps, obj_maps
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_entity(self, tgt_entity, entity_pos,
    ...
    def forward_triplet(self, tgt_triplet, tgt_entity, triplet_pos, so_pos,
                        memory, tgt_mask: Optional[Tensor] = None,
                        memory_mask: Optional[Tensor] = None,
                        tgt_key_padding_mask: Optional[Tensor] = None,
                        memory_key_padding_mask: Optional[Tensor] = None,
        ...
        return tgt_triplet, sub_maps, obj_maps
    ...
    def forward(self, tgt_entity, tgt_triplet, entity_pos, triplet_pos, so_pos,
                ...
                memory_key_padding_mask: Optional[Tensor] = None,
                pos: Optional[Tensor] = None):
        tgt_entity = self.forward_entity(
            tgt_entity, entity_pos, memory, tgt_mask=tgt_mask, memory_mask=memory_mask,
            tgt_key_padding_mask=tgt_key_padding_mask, memory_key_padding_mask=memory_key_padding_mask,
            pos=pos)
        tgt_triplet, sub_maps, obj_maps = self.forward_triplet(
            tgt_triplet, tgt_entity, triplet_pos, so_pos, memory, tgt_mask=tgt_mask,
